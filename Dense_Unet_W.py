...
class Network:
    ...
    def train(self,configure):
        ...
        cross_loss = tf.reduce_mean(-tf.reduce_mean(w * Y_ * tf.log(Y_pred_modi_ + 1e-8), reduction_indices=[1]) -
                                tf.reduce_mean((1 - w) * (1 - Y_) * tf.log(1 - Y_pred_modi_ + 1e-8),
                                               reduction_indices=[1]))
        loss_sum = tf.summary.scalar("cross entropy",cross_loss)

        # trainers
        optim = tf.train.AdamOptimizer(learning_rate=lr, beta1=0.9, beta2=0.999, epsilon=1e-8).minimize(cross_loss)
                        ...
                        for i in range(0, len(block_numbers), test_batch_size):
                            ...
                            if i + test_batch_size < len(block_numbers):
                                ...
                                X_temp = tf.placeholder(
                                    ...
                                    dtype=tf.float32)
                                with tf.variable_scope('segmenter', reuse=True):
